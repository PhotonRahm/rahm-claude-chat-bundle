...
| Kalshi Crypto DS | deterministic_settlement | Final-window crypto cushion settlement on BTC/ETH, per-asset sizing BTC qty 700 / ETH qty 300, daily CB -$700, lifetime CB -$2,100, max_open 8, max 4 open positions per asset/settlement-hour, combined filter price >=0.93 and cushion >=0.10%, qty and filter cohort trackers active | Proper cushion DS with $0.93 min-price filter active. On 2026-05-13 the operator elected BTC qty 700 after the qty-600 redeploy cohort cleared cleanly; active cohorts are `filter_combined_qty_700_btc_redeploy_pilot` and `filter_combined_qty_300_eth_pilot`. Hard ceiling at qty 700; no qty 800+ without fresh CVaR and clean qty-700 cohort at n>=50. | `KILL_DETERMINISTIC_SETTLEMENT` |
