...
| Kalshi Mean Reversion | mean_reversion | Prediction-market mean reversion with surgical cell controls: KXHIGH/KXETHD/KXBTCD retired via MR-only pattern blocks, ETH fully blocked, WEATHER_LOW fully blocked, SOL fully blocked, INDEX capped at max 2 per underlier/event and max 4 total INDEX positions per settlement hour, spread $0.02-$0.05, and qty 10 data-collection sizing | Live MR minimal-sizing cohort from 2026-05-13. If post-deploy n>=30 is negative P&L, hard kill permanently; if positive with Wilson lower above breakeven, operator may evaluate larger sizing. | Kalshi MR kill paths in repo CLAUDE.md |
| Kalshi Crypto DS | deterministic_settlement | Final-window crypto cushion settlement on BTC/ETH, per-asset sizing BTC qty 600 / ETH qty 300, daily CB -$600, lifetime CB -$1,800, max_open 8, max 4 open positions per asset/settlement-hour, max 3 total placements per contract-hour, combined filter price >=0.93 and cushion >=0.10%, qty and filter cohort trackers active | Proper cushion DS with $0.93 min-price filter active. On 2026-05-13 the operator rolled BTC back from qty 700 to qty 600 after the qty-700 redeploy cohort triggered the pre-committed rollback rule, then deployed the statistically supported contract-hour max-3 cluster cap from the DS loss forensic. Active cohorts are `filter_combined_qty_600_btc_post_rollback_pilot`, `filter_combined_qty_300_eth_pilot`, and `kalshi_ds_contract_hour_max3_forward_pilot`. Future qty-700+ attempts are not permanently prohibited, but require fresh operator decision, CVaR, and evidence. | `KILL_DETERMINISTIC_SETTLEMENT` |
| Kalshi Index Longshot Fade | deterministic_settlement_index | Cheap-side longshot fade using audited TTE-bucketed heuristic rows | PERMANENT_KILL 2026-05-13: first live cohort 0W-4L, -$8.19; no revival authorized | `KILL_DETERMINISTIC_SETTLEMENT_INDEX` |
| Kalshi FX Longshot Fade | deterministic_settlement_fx | Cheap-side longshot fade using audited TTE-bucketed heuristic rows | PERMANENT_KILL 2026-05-13: first live cohort 0W-8L, -$18.41; no revival authorized | `KILL_DETERMINISTIC_SETTLEMENT_FX` |
| Kalshi Moderate Favorites | moderate_favorites | Narrow-band favorite pilot | PERMANENT_KILL 2026-05-13: 154W-50L, -$209.49; 2026-05-11 forensic Verdict B; no revival authorized | `KILL_MODERATE_FAVORITES` |
| IBKR Long+Short-dated | bot_strategy | ForecastEx economics/FX filter stack | Live IBKR strategy | IBKR repo CLAUDE.md |
| IBKR Weather DS Per-Cell Pilots | deterministic_settlement_weather | NWS-cushion weather settlement pilots on live cells `UHBNA_YES_cushion_2_to_5f` and `UHPHX_YES_cushion_2_to_5f`; qty 10, max_open 2 per live cell, per-cell daily/lifetime CB -$75/-$150, aggregate rollback daily/lifetime -$20/-$30. `edge_threshold=0.40` means `model_probability - ask >= 0.40`, effectively ask <= $0.40 for current model 0.80 cells. `UHCLT_YES_cushion_2_to_5f`, `UHJAX_YES_cushion_lt_2f`, and `UHSFO_YES_cushion_2_to_5f` are excluded from live placement after AV. Dropped cells `UHATL_YES_cushion_lt_2f`, `UHPHL_YES_cushion_lt_2f`, and `UHLAX_YES_cushion_lt_2f` are shadow-tracked in `ds_weather_shadow_signals` with graduation criteria n>=30 forward shadow, WR>=90%, Wilson lower>=80%, positive hypothetical P&L. Historical Weather DS rows are not inherited. | Post-fix bounded reactivation from 2026-05-16 20:55:30 UTC after AT signed-cushion fix and favorable-cushion collapsed metrics; pre-fix rows 192-194 excluded and left to settle naturally | `KILL_DETERMINISTIC_SETTLEMENT_WEATHER` |
